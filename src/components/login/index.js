...
class Login extends Component {
    ...
    handleChange = (event) => {
        ...
        switch (name) {
            case "email":
                errors.email =
                    value.length == 0 ? "": validEmailRegex.test(value)? "": "Email is not valid!(Must Contain @ ,.com)";
                break;
            case "password":
                errors.password =
                    value.length == 0 ? "": validpasswordRegex.test(value)? "": "Password is not valid!(Must Contain at least 8 characters and $ , a-z ,A-z)";
                break;
        }
